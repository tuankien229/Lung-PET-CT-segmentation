...
class UpPPP(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, x5):
        ...
        return self.conv(x)
class UpPPPP(nn.Module):
    def __init__(self, in_channels, out_channels, trilinear=True):
        super().__init__()
        self.up = nn.ConvTranspose3d(in_channels, out_channels, kernel_size=2, stride=2)
        self.conv = DoubleConv(out_channels*6, out_channels)
    def forward(self, x1, x2, x3, x4, x5, x6):
        x1 = self.up(x1)
        # Make sure size data when you combine are the same size
        diffX1 = x6.size()[4] - x1.size()[4]
        diffY1 = x6.size()[3] - x1.size()[3]
        diffZ1 = x6.size()[2] - x1.size()[2]
        
        diffX2 = x6.size()[4] - x2.size()[4]
        diffY2 = x6.size()[3] - x2.size()[3]
        diffZ2 = x6.size()[2] - x2.size()[2]
        
        diffX3 = x6.size()[4] - x3.size()[4]
        diffY3 = x6.size()[3] - x3.size()[3]
        diffZ3 = x6.size()[2] - x3.size()[2]
        
        diffX4 = x6.size()[4] - x4.size()[4]
        diffY4 = x6.size()[3] - x4.size()[3]
        diffZ4 = x6.size()[2] - x4.size()[2]
        
        diffX5 = x6.size()[4] - x5.size()[4]
        diffY5 = x6.size()[3] - x5.size()[3]
        diffZ5 = x6.size()[2] - x5.size()[2]
        
        x4 = torch.nn.functional.pad(x5, (diffX5 // 2, diffX5 - diffX5 // 2, diffY5 // 2, diffY5 - diffY5 // 2, diffZ5 // 2, diffZ5 - diffZ5 // 2))  
        x4 = torch.nn.functional.pad(x4, (diffX4 // 2, diffX4 - diffX4 // 2, diffY4 // 2, diffY4 - diffY4 // 2, diffZ4 // 2, diffZ4 - diffZ4 // 2))        
        x3 = torch.nn.functional.pad(x3, (diffX3 // 2, diffX3 - diffX3 // 2, diffY3 // 2, diffY3 - diffY3 // 2, diffZ3 // 2, diffZ3 - diffZ3 // 2))
        x2 = torch.nn.functional.pad(x2, (diffX2 // 2, diffX2 - diffX2 // 2, diffY2 // 2, diffY2 - diffY2 // 2, diffZ2 // 2, diffZ2 - diffZ2 // 2))
        x1 = torch.nn.functional.pad(x1, (diffX1 // 2, diffX1 - diffX1 // 2, diffY1 // 2, diffY1 - diffY1 // 2, diffZ1 // 2, diffZ1 - diffZ1 // 2))
